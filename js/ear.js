...
var Ear = (function () {
        ...
        navigator.getUserMedia({
            ...
            this.indices = _.chain(options.bands)
                ...
                .map(function (band) {
                    ...
                    return Math.round((2 * new_freq * this.bufSize) / this.ctx.sampleRate);
                }, this)
    ...
    Ear.prototype.getBand = function (frequency) {
        var start = 10000, width = 500, i;

        if (frequency) {
            // Find band that this frequency belongs to
            for (var band in this.bands) {
                if (this.bands.hasOwnProperty(band) && this.bands[band].contains(frequency)) {
